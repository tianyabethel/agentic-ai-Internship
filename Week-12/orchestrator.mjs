...
export async function orchestrate(message, options = {}) {
  ...
  if (intents.includes("search")) {
    ...
    const cityMatch = message.match(/\b(?:in|near|around)\s+([A-Za-z]+(?:\s+[A-Za-z]+)?)/i);

    if (!filters.city && cityMatch) {
      filters.city = cityMatch[1].trim();
    }

    if (filters.maxPrice == null && /\baffordable\b/i.test(message)) {
      filters.maxPrice = 500000;
    }

    const searchResults = await searchActiveListings(filters);
  ...
  if (intents.includes("email")) {
    results.email = await emailDraftAgent(message);
  }
