...
## Week 10 Real Estate Orchestrator

For real estate requests, use the Week 10 orchestration workflow in `orchestrator.mjs`.

The orchestrator coordinates property search, market statistics, recommendations, RAG knowledge, and email drafting.

When a user sends a real estate request:
1. Use the orchestrator to classify the request and call the appropriate agents.
2. Respect the guardrails in `guardrails.mjs`.
3. Do not fabricate listings, market data, recommendations, or knowledge answers.
4. Return clear user-facing errors when required data or services are unavailable.
5. For WhatsApp, keep responses concise and avoid markdown tables.
